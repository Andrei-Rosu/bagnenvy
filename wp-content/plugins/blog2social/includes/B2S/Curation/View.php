...
class B2S_Curation_View {
    ...
    public function getCurationPreviewHtml($url = '', $data = array()) {
        ...
        $html .='<input type="hidden" id="b2s-post-curation-image-url" name="image_url" value="' . ($externalImage ? esc_url($image) : "") . '">';
    ...
    public function getShippingDetails($mandant = array(), $auth = array()) {
        ...
        $content .='</div>';
        $content .='<div class="col-xs-12 col-sm-5 col-lg-3">';
        $content .='<label for="b2s-curation-profile-select">' . esc_html__('Select network collection:', 'blog2social') . '</label>';
